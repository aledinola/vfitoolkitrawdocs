...
Just some simple backward iteration. Let $\Theta$ be all the parameters of the model, and let $\theta_j$ be the values of those parameters which are relevant in period $j$.\footnote{So if a parameter is independent of age, then it is just in $\theta_j$ as is. If a parameter depends on age, then only the value relevant to age $j$ is in $\theta_j$.}
\begin{algorithmic}
 ...
 \For $j$ count backward from $N_j-1$ to $1$
   ...
   \For All values $z$
    ...
    \For All values of $(a,z)$
     \State Calculate $V_j(a,z)=max_{d,a' \in D(a,z)} F(d,a',a,z)+\beta E[V_{j+1}(a',z')]$
...
\begin{algorithmic}
 ...
 \For $j$ count backward from $N_j-1$ to $1$
   \State Get $\theta_j$ from $\Theta$
   \For All values $z$
    \State Calculate $E[V_{j+1}(a',semiz',z')|d]$ \Comment Note conditional on d, because of semi-exog state
    \For All values of $(a,semiz,z)$
     \State Calculate $V_j(a,semiz,z)=max_{d,a' \in D(a,semiz,z)} F(d,a',a,semiz,z)+\beta E[V_{j+1}(a',semiz',z')|d]$
     \State ... and keep the $argmax$ $g_j(a,semiz,z)$.
     \EndFor
   \EndFor
  \EndFor
   \\ \Return $[V_1,V_2,...,V_{N_j}]$, $[g_1,g_2,...,g_{N_j}]$
...
The $E[V_{j+1}(a',semiz',z')|d]$ term is doing a lot of work here. It adds a whole dimension to the expectations term. The way this is implemented in the codes is actually slightly different, namely there is a loop over $d$ and the problem is solved conditional on d, then once the loop is finished the maximum over d is taken. That is the lines 5-11 become
\begin{algorithmic}
  \For All values of $d$
   \For All values $z$
    \State Calculate $E[V_{j+1}(a',semiz',z')|d]$ \Comment Note conditional on d, because of semi-exog state
    \For All values of $(a,semiz,z)$
     \State Calculate $V_j(a,semiz,z|d)=max_{a' \in D(a,semiz,z)} F(d,a',a,semiz,z)+\beta E[V_{j+1}(a',semiz',z')|d]$
     \State ... and keep the $argmax$ $g_j(a,semiz,z|d)$.
     \EndFor
   \EndFor
  \EndFor
  \State $V_j(a,semiz,z)=\max_{d} V_j(a,semiz,z|d)$
  \State and used the $argmax$ $d$ from this, denoted $d*$ to get $g_j(a,z)=g_j(a,z|d*)$.
\end{algorithmic}
...
VFI Toolkit uses linear interpolation as part of alternative endogenous states like experienceasset, experienceassetu, riskyasset, and more. There are two halves to how the toolkit uses interpolation. One half is to put points onto grid (as a lower grid point index and a probability of the lower point; together implicitly with an upper grid point and probability of the upper point). The second half is to use this to 'switch' grids.

For example in the previous pseudocode we saw the first half as: \\
\textit{Interpolate $aprime(d,a)$ onto the grid for $a$; get $aprime_i(d,a)$ and $aProbs(d,a)$, the index for the lower grid point and the probability of the lower grid point.}
\\ The way that the toolkit interpolates a value onto the grid is to uses linear interpolation. So say we have that the value we want to interpolate onto the grid is $x$, and the grid is $\{x_1,x_2,\dots,x_n\}$. The first step is to find the two grid points just either side of $x$, call them $x_i$ and $x_{i+1}$, and these will obviously satisfy $x_i\leq x < x_{i+1}$ [note equality on the lower but not upper point; this is arbitrary, just pick one]. Then we call $x_i$ the lower grid point. We assign this lower grid point the probability $1-\frac{(x-x_i)}{(x_{i+1}-x_i)}$ (notice that this is a number from 0 to 1, and is 1 if $x=x_i$). We only need to store the (index of the) lower grid point and probability of the lower grid point, as we always know that the upper grid point is just the point one above this, and that the probability of the upper grid point is one minus the probability of the lower grid point (so probability of upper grid point is $\frac{(x-x_i)}{(x_{i+1}-x_i)}$, which again is 0 to 1, and approaches 1 as $x$ approaches $x_{i+1}$.
...
\\ The way the toolkit does this is that for every $(d,a)$ we have a value $aprime(d,a)$ which we associated in the first half with a lower grid point index and probability, $aprime_i(d,a)$ and $aProbs(d,a)$. Looking at $E[V_{j+1}(a',z')]$ and ignoring the $E[]$ for the moment, we get \\ $V_{j+1}(a',z')=aProbs(d,a) V_{j+1}(aprime_i(d,a),z')+ (1-aProbs(d,a)) V_{j+1}(aprime_{i+1}(d,a),z')\equiv V_{j+1}(d,a,z')$. We just need to do this before we evaluate the expectations term, and we will thus get $E[V_{j+1}(d,a,z')]$.
